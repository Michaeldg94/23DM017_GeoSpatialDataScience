...
$call gdxxrw i=resultsFG.xlsx o=imp_varq.gdx par=imp_varq rng="q!a1" cdim=1 rdim=2
$gdxin imp_varq.gdx
$load imp_varq
q(m,c,y) = imp_varq(m,c,y);

$call gdxxrw i=resultsFG.xlsx o=imp_varp.gdx par=imp_varp rng="p!a1" cdim=1 rdim=2
$gdxin imp_varp.gdx
$load imp_varp
p(m,c,y) = imp_varp(m,c,y);

$call gdxxrw i=resultsFG.xlsx o=imp_vartr.gdx par=imp_vartr rng="tr!a1" cdim=1 rdim=3
$gdxin imp_vartr.gdx
$load imp_vartr
...
$call gdxxrw i=farmgate.xlsx o=imp_vary.gdx par=imp_vary rng="y!a1" cdim=1 rdim=1
$gdxin imp_vary.gdx
$load imp_vary
inc(m,y) = imp_vary(m,y);
...
Parameter nprod(m,y) production of numeraire good FINAL;
nprod(m,y)$(afri(m)) = inc(m,y) - sum(c,p(m,c,y)*q(m,c,y)) - sum(n,sum(c,p(m,c,y)*(tr(m,n,c,y)-tr(n,m,c,y)))) - sum(n,sum(c,0.5*tau(m,n)*(tr(m,n,c,y)+tr(n,m,c,y))));
Parameter nprod2(m,y) production of numeraire good TOTAL (includes intermediates for storage and trade costs);
nprod2(m,y)$(afri(m)) = inc(m,y) - agrev(m,y);
...
tcostTin(m,"tcostT") = sum(y,tcost(m,y));

*Parameter scostTin(m,varname);
*scostTin(m,"scostT") = sum(y,scost(m,y));

Parameter agrevTin(m,varname);
agrevTin(m,"agrevT") = sum(y,agrev(m,y));

Parameter nprodTin(m,varname);
nprodTin(m,"nprodT") = sum(y,nprod(m,y));

Parameter agconsTin(m,varname);
agconsTin(m,"agconsT") = sum(y,agcons(m,y));

Parameter agquanTin(m,varname);
agquanTin(m,"agquanT") = sum(y,agquan(m,y));

Parameter qindexAVGin(m,varname);
qindexAVGin(m,"qindexAVG") = sum(y,qindex(m,y))/10;

Parameter nconsTin(m,varname);
nconsTin(m,"nconsT") = sum(y,ncons(m,y));

Parameter nexpTin(m,varname);
nexpTin(m,"nexpT") = sum(y,nexp(m,y));

Parameter agtrdomTin(m,varname);
agtrdomTin(m,"agtrdomT") = sum(y,agtrdom(m,y));

Parameter agtrintlTin(m,varname);
agtrintlTin(m,"agtrintlT") = sum(y,agtrintl(m,y));

Parameter agtrtotTin(m,varname);
agtrtotTin(m,"agtrtotT") = sum(y,agtrtot(m,y));

Parameter agtrnetTin(m,varname);
agtrnetTin(m,"agtrnetT") = sum(y,agtrnet(m,y));

Parameter pindexAVGin(m,varname);
pindexAVGin(m,"pindexAVG") = sum(y,pindex(m,y))/10;

Parameter utilTin(m,varname);
utilTin(m,"utilT") = sum(y,util(m,y));
...
execute_unload "welfare" tcostTin agrevTin nprodTin agconsTin agquanTin qindexAVGin nconsTin nexpTin agtrdomTin agtrintlTin agtrtotTin agtrnetTin pindexAVGin utilTin;
