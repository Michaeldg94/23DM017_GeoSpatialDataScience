...
foreach x of varlist tcost* agrev* nexp* agtr*{
replace `x' = 0 if `x' < 0.1 & `x' > 0
}

gen incomet = agconst + nconst

foreach x of varlist tcostt agrevt incomet agconst agquant qindexavg nconst nexpt agtrdomt agtrintlt agtrtott agtrnett pindexavg utilt {
gen `x'diff = `x'cf - `x'
gen `x'hat = `x'cf/`x'
}
