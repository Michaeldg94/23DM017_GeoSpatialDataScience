...
Set y years /2003*2012/;

*Parameter s(m,c,t) storage;
Parameter q(m,c,y) consumption;
Parameter p(m,c,y) price;
Parameter tr(m,n,c,y) trade;

Parameter imp_varq(m,c,y), imp_varp(m,c,y), imp_vartr(m,n,c,y);

$call gdxxrw i=resultsFGcf.xlsx o=imp_varq.gdx par=imp_varq rng="q!a1" cdim=1 rdim=2
$gdxin imp_varq.gdx
$load imp_varq
q(m,c,y) = imp_varq(m,c,y);

$call gdxxrw i=resultsFGcf.xlsx o=imp_varp.gdx par=imp_varp rng="p!a1" cdim=1 rdim=2
$gdxin imp_varp.gdx
$load imp_varp
p(m,c,y) = imp_varp(m,c,y);

$call gdxxrw i=resultsFGcf.xlsx o=imp_vartr.gdx par=imp_vartr rng="tr!a1" cdim=1 rdim=3
$gdxin imp_vartr.gdx
$load imp_vartr
tr(m,n,c,y) = imp_vartr(m,n,c,y);

Parameter nprod2(m,y) TOTAL production of numeraire good;
Parameter imp_vary(m,y);

$call gdxxrw i=in_nprod.xlsx o=imp_vary.gdx par=imp_vary rng="Sheet1!a1" cdim=1 rdim=1
$gdxin imp_vary.gdx
$load imp_vary
*nprod(m,t) = imp_vary(m,t);
nprod2(m,y) = imp_vary(m,y);
...
Parameter h(m,c,y) harvest;
Parameter imp_varh(m,c,y);

$call gdxxrw i=farmgate.xlsx o=imp_varh.gdx par=imp_varh rng="h!a1" cdim=1 rdim=2
$gdxin imp_varh.gdx
$load imp_varh
h(m,c,y) = imp_varh(m,c,y);

Parameter Pop(m,y) population;
Parameter imp_varn(m,y);

$call gdxxrw i=farmgate.xlsx o=imp_varn.gdx par=imp_varn rng="n!a1" cdim=1 rdim=1
$gdxin imp_varn.gdx
$load imp_varn
Pop(m,y) = imp_varn(m,y);
...
Parameter tcost(m,y) expenditure on trade costs;
tcost(m,y)$(afri(m)) = sum(n,sum(c,0.5*tau(m,n)*(tr(m,n,c,y)+tr(n,m,c,y))));
...
Parameter agrev(m,y) ag revenue net of storage and trade costs;
agrev(m,y)$(afri(m)) = sum(c,p(m,c,y)*h(m,c,y));

Parameter inc(m,y) income;
inc(m,y)$(afri(m)) = nprod2(m,y) + agrev(m,y);

Parameter agcons(m,y) expenditure on grains;
agcons(m,y)$(afri(m)) = sum(c,p(m,c,y)*q(m,c,y));

Parameter agquan(m,y) quantity of grains consumed;
agquan(m,y)$(afri(m)) = sum(c,q(m,c,y));

Parameter qindex(m,y) quantity index of grains;
qindex(m,y)$(afri(m)) = prod(c$cr(m,c),((q(m,c,y)/alpha(m,c)$cr(m,c))**alpha(m,c)$cr(m,c)));

Parameter ncons(m,y) consumption of numeraire good;
ncons(m,y)$(afri(m)) = inc(m,y) - agcons(m,y);

Parameter nexp(m,y) net exports of numeraire good;
nexp(m,y)$(afri(m)) = - sum(n,sum(c,p(m,c,y)*(tr(m,n,c,y)-tr(n,m,c,y)))) - sum(n,sum(c,0.5*tau(m,n)*(tr(m,n,c,y)+tr(n,m,c,y))));

Parameter agtrdom(m,y) quantity of grains traded internally;
agtrdom(m,y)$(afri(m)) = sum(n,sum(c,0.5*(tr(m,n,c,y)$(afri(n))+tr(n,m,c,y)$(afri(n)))));

Parameter agtrintl(m,y) quantity of grains traded with world;
agtrintl(m,y)$(afri(m)) = sum(n,sum(c,(tr(m,n,c,y)$(not afri(n))+tr(n,m,c,y)$(not afri(n)))));

Parameter agtrtot(m,y) total quantity of grains traded;
agtrtot(m,y)$(afri(m)) = agtrdom(m,y) + agtrintl(m,y);

Parameter agtrnet(m,y) net quantity of grains traded (net exports);
agtrnet(m,y)$(afri(m)) = sum(n,sum(c,tr(m,n,c,y)-tr(n,m,c,y)));

Parameter pindex(m,y) price index of grains;
pindex(m,y)$(afri(m)) = prod(c,p(m,c,y)**alpha(m,c));

Parameter util(m,y) indirect utility;
util(m,y) = inc(m,y)$(afri(m)) - (1/(epsi+1))*A(m)*Pop(m,y)*(pindex(m,y)**(epsi+1));
...
tcostTin(m,"tcostTcf") = sum(y,tcost(m,y));

*Parameter scostTin(m,varname);
*scostTin(m,"scostT") = sum(y,scost(m,y));

Parameter agrevTin(m,varname);
agrevTin(m,"agrevTcf") = sum(y,agrev(m,y));

Parameter yTin(m,varname);
yTin(m,"incomeTcf") = sum(y,inc(m,y));

Parameter agconsTin(m,varname);
agconsTin(m,"agconsTcf") = sum(y,agcons(m,y));

Parameter agquanTin(m,varname);
agquanTin(m,"agquanTcf") = sum(y,agquan(m,y));

Parameter qindexAVGin(m,varname);
qindexAVGin(m,"qindexAVGcf") = sum(y,qindex(m,y))/10;

Parameter nconsTin(m,varname);
nconsTin(m,"nconsTcf") = sum(y,ncons(m,y));

Parameter nexpTin(m,varname);
nexpTin(m,"nexpTcf") = sum(y,nexp(m,y));

Parameter agtrdomTin(m,varname);
agtrdomTin(m,"agtrdomTcf") = sum(y,agtrdom(m,y));

Parameter agtrintlTin(m,varname);
agtrintlTin(m,"agtrintlTcf") = sum(y,agtrintl(m,y));

Parameter agtrtotTin(m,varname);
agtrtotTin(m,"agtrtotTcf") = sum(y,agtrtot(m,y));

Parameter agtrnetTin(m,varname);
agtrnetTin(m,"agtrnetTcf") = sum(y,agtrnet(m,y));

Parameter pindexAVGin(m,varname);
pindexAVGin(m,"pindexAVGcf") = sum(y,pindex(m,y))/10;

Parameter utilTin(m,varname);
utilTin(m,"utilTcf") = sum(y,util(m,y));
...
execute_unload "welfare" tcostTin agrevTin yTin agconsTin agquanTin qindexAVGin nconsTin nexpTin agtrdomTin agtrintlTin agtrtotTin agtrnetTin pindexAVGin utilTin;
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=utilTin rng=cf!o1:p230 cdim=1 rdim=1'
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=pindexAVGin rng=cf!n1:o230 cdim=1 rdim=1'
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=agtrnetTin rng=cf!m1:n230 cdim=1 rdim=1'
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=agtrtotTin rng=cf!l1:m230 cdim=1 rdim=1'
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=agtrintlTin rng=cf!k1:l230 cdim=1 rdim=1'
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=agtrdomTin rng=cf!j1:k230 cdim=1 rdim=1'
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=nexpTin rng=cf!i1:j230 cdim=1 rdim=1'
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=nconsTin rng=cf!h1:i230 cdim=1 rdim=1'
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=qindexAVGin rng=cf!g1:h230 cdim=1 rdim=1'
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=agquanTin rng=cf!f1:g230 cdim=1 rdim=1'
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=agconsTin rng=cf!e1:f230 cdim=1 rdim=1'
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=yTin rng=cf!d1:e230 cdim=1 rdim=1'
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=agrevTin rng=cf!c1:d230 cdim=1 rdim=1'
*execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=scostTin rng=cf!b1:c230 cdim=1 rdim=1'
execute 'gdxxrw.exe welfare.gdx o=welfare.xlsx par=tcostTin rng=cf!a1:b230 cdim=1 rdim=1'
$exit
