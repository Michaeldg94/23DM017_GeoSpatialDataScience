...
$call gdxxrw i=EP44.xlsx o=imp_varmt1.gdx par=imp_varmt1 rng="EP4e!a1:ec230" cdim=1 rdim=1
$gdxin imp_varmt1.gdx
$load imp_varmt1
Pop(m,t) = imp_varmt1(m,t);

$call gdxxrw i=EH44.xlsx o=imp_varmct.gdx par=imp_varmct rng="EH4e!a1:ed1375" cdim=1 rdim=2
...
$call gdxxrw i=results6A15in.xlsx o=imp_vars0.gdx par=imp_vars0 rng="s0!a1:c400" cdim=1 rdim=2
$gdxin imp_vars0.gdx
$load imp_vars0
...
$call gdxxrw i=in_pint.xlsx o=imp_pint.gdx par=imp_pint rng="PI16!a1:ed6" cdim=1 rdim=2
$gdxin imp_pint.gdx
$load imp_pint
...
$call gdxxrw i=results6A15in.xlsx o=imp_pin.gdx par=imp_pin rng="p!a1:bj512" cdim=1 rdim=2
$gdxin imp_pin.gdx
$load imp_pin
$call gdxxrw i=results6A15in.xlsx o=imp_qin.gdx par=imp_qin rng="q!a1:bj512" cdim=1 rdim=2
$gdxin imp_qin.gdx
$load imp_qin
$call gdxxrw i=results6A15in.xlsx o=imp_sin.gdx par=imp_sin rng="s!a1:bi512" cdim=1 rdim=2
$gdxin imp_sin.gdx
$load imp_sin
$call gdxxrw i=results6A15in.xlsx o=imp_trin.gdx par=imp_trin rng="tr!a1:bk1000" cdim=1 rdim=3
$gdxin imp_trin.gdx
$load imp_trin
...
model africa6A16_MCP / quantity.p, price.q , storagenoarb.s, tradenoarb.tr/;

africa6A16_MCP.optfile = 1;

solve africa6A16_MCP using MCP ;

execute_unload "results6A16" s.l p.l q.l;
execute_unload "resultstr6A16" tr.l;
execute 'gdxxrw.exe results6A16.gdx o=results6A16.xlsx var=s.l rng=s!a2:bi520'
execute 'gdxxrw.exe results6A16.gdx o=results6A16.xlsx var=s.l rng=s0!a1'
execute 'gdxxrw.exe results6A16.gdx o=results6A16.xlsx var=p.l rng=p!a2:bj520'
execute 'gdxxrw.exe results6A16.gdx o=results6A16.xlsx var=q.l rng=q!a2:bj520'
execute 'gdxxrw.exe resultstr6A16.gdx o=results6A16.xlsx var=tr.l rng=tr!a2:bk1000'
...
execute_unload "results6A16in" storin pricin quanin;
execute_unload "resultstr6A16in" tradin;
execute_unload "resultsS06A16in" s0in;
execute 'gdxxrw.exe results6A16in.gdx o=results6A16in.xlsx par=storin rng=s!a1 cdim=1 rdim=2'
execute 'gdxxrw.exe resultsS06A16in.gdx o=results6A16in.xlsx par=s0in rng=s0!a1 cdim=1 rdim=2'
execute 'gdxxrw.exe results6A16in.gdx o=results6A16in.xlsx par=pricin rng=p!a1 cdim=1 rdim=2'
execute 'gdxxrw.exe results6A16in.gdx o=results6A16in.xlsx par=quanin rng=q!a1 cdim=1 rdim=2'
execute 'gdxxrw.exe resultstr6A16in.gdx o=results6A16in.xlsx par=tradin rng=tr!a1 cdim=1 rdim=3'
